...
<a href="<?php echo esc_url( $product['url'] ); ?>" class="tag tag--link flex h-full flex-col px-5 pb-5 pt-11 text-ink no-underline"
	...
	<span class="stencil block text-[1.55rem]" <?php nwcs_edit_attr( $product['key'], 'card', 'mark' ); ?>><?php echo esc_html( $product['mark'] ); ?></span>

	<span class="shot mt-4 block aspect-[16/10]" <?php nwcs_edit_attr( $product['key'], 'card', 'image' ); ?>>
		<?php echo kr_image_tag( $product['image'], '', $product['name'] ); // phpcs:ignore WordPress.Security.EscapingOutput ?>
	</span>

	<span class="mt-4 block font-slab text-[1.35rem] font-bold leading-tight" <?php nwcs_edit_attr( $product['key'], 'card', 'name' ); ?>><?php echo esc_html( $product['name'] ); ?></span>
	<span class="mt-1.5 block text-[0.9375rem] leading-snug text-muted" <?php nwcs_edit_attr( $product['key'], 'card', 'short' ); ?>><?php echo esc_html( $product['short'] ); ?></span>
</a>
