		...
		<section id="teklif" class="card mx-auto max-w-[46rem] scroll-mt-28 p-7 md:p-10">
			...
			<?php else : ?>
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="mt-8 grid gap-6 sm:grid-cols-2" novalidate>
					<input type="hidden" name="action" value="ahsapkasa_quote" />
					<?php wp_nonce_field( 'ahsapkasa_quote', 'ahsapkasa_quote_nonce' ); ?>

					<p class="hidden" aria-hidden="true">
						<label>Bu alanı boş bırakın
							<input type="text" name="ahsapkasa_website" tabindex="-1" autocomplete="off" />
						</label>
					</p>

					<div>
						<label for="ak_name" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'name_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'name_label' ) ); ?>
						</label>
						<input type="text" id="ak_name" name="ak_name" required
							value="<?php echo esc_attr( $values['name'] ?? '' ); ?>"
							<?php echo isset( $errors['name'] ) ? 'aria-describedby="ak_name_err" aria-invalid="true"' : ''; ?>
							class="<?php echo esc_attr( isset( $errors['name'] ) ? $broken : $field ); ?>" />
						<?php if ( isset( $errors['name'] ) ) : ?>
							<p id="ak_name_err" class="mt-2 text-sm text-alert"><?php echo esc_html( $errors['name'] ); ?></p>
						<?php endif; ?>
					</div>

					<div>
						<label for="ak_company" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'company_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'company_label' ) ); ?>
						</label>
						<input type="text" id="ak_company" name="ak_company"
							value="<?php echo esc_attr( $values['company'] ?? '' ); ?>"
							class="<?php echo esc_attr( $field ); ?>" />
					</div>

					<div>
						<label for="ak_email" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'email_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'email_label' ) ); ?>
						</label>
						<input type="email" id="ak_email" name="ak_email" required
							value="<?php echo esc_attr( $values['email'] ?? '' ); ?>"
							<?php echo isset( $errors['email'] ) ? 'aria-describedby="ak_email_err" aria-invalid="true"' : ''; ?>
							class="<?php echo esc_attr( isset( $errors['email'] ) ? $broken : $field ); ?>" />
						<?php if ( isset( $errors['email'] ) ) : ?>
							<p id="ak_email_err" class="mt-2 text-sm text-alert"><?php echo esc_html( $errors['email'] ); ?></p>
						<?php endif; ?>
					</div>

					<div>
						<label for="ak_phone" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'phone_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'phone_label' ) ); ?>
						</label>
						<input type="tel" id="ak_phone" name="ak_phone"
					...
					<div class="sm:col-span-2">
						<label for="ak_size" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'size_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'size_label' ) ); ?>
						</label>
						<textarea id="ak_size" name="ak_size" rows="2" required
							placeholder="<?php echo esc_attr( nwcs_field( 'contact', 'form', 'size_hint' ) ); ?>"
							<?php echo isset( $errors['size'] ) ? 'aria-describedby="ak_size_err" aria-invalid="true"' : ''; ?>
						...
						<?php if ( isset( $errors['size'] ) ) : ?>
							<p id="ak_size_err" class="mt-2 text-sm text-alert"><?php echo esc_html( $errors['size'] ); ?></p>
						<?php endif; ?>
					</div>

					<div class="sm:col-span-2">
						<label for="ak_message" class="<?php echo esc_attr( $label ); ?>" <?php nwcs_edit_attr( 'contact', 'form', 'message_label' ); ?>>
							<?php echo esc_html( nwcs_field( 'contact', 'form', 'message_label' ) ); ?>
						</label>
						<textarea id="ak_message" name="ak_message" rows="4" class="<?php echo esc_attr( $field ); ?>"><?php echo esc_textarea( $values['message'] ?? '' ); ?></textarea>
					</div>
